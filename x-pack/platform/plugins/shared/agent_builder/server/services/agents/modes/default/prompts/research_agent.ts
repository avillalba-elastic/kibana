...
const getAgentSystemMessage = async ({
  ...
  const remainingSteps = cycleLimit - currentCycle;
...
## REASONING BUDGET
You have **${remainingSteps} step(s) remaining** (out of ${cycleLimit} total). A step is one LLM decision — it may include multiple parallel tool calls, but they count as a single step. This limit is enforced externally: when it is reached you will be forced to hand over regardless of completeness.

Prioritize the most critical tool calls first. Use parallel calls to maximize information per step. If you are approaching the limit, finalize with what you have rather than starting a new line of inquiry.
